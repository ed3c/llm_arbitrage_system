...
def parse_lineage_manifest(payload: Mapping[str, Any]) -> DatasetLineageManifest:
    ...
    if isinstance(schema_version, bool) or schema_version != _LINEAGE_SCHEMA_VERSION:
        raise ValueError("lineage schema_version must be 1")

    dataset_hash = _sha256(
        payload.get("dataset_semantic_sha256"),
        "dataset_semantic_sha256",
    )
    ...
    if operation_unknown:
        raise ValueError(
            "lineage operation contains unknown fields: "
            + ", ".join(operation_unknown)
        )
