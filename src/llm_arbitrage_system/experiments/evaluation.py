...
class ExperimentMatrixSnapshot:
    ...
    def evaluation(self, evaluation_id: str) -> MatrixEvaluation:
        ...
        if len(matches) != 1:
            raise ValueError(
                f"matrix must contain evaluation_id exactly once: {evaluation_id}"
            )
        return matches[0]
...
def load_experiment_matrix(path: Path) -> ExperimentMatrixSnapshot:
    ...
    payload = _load_json_object(resolved, "experiment matrix")
    allowed = {
        "schema_version",
    ...
    if unknown:
        raise ValueError(f"matrix contains unknown fields: {', '.join(unknown)}")
    schema_version = payload.get("schema_version")
    if isinstance(schema_version, bool) or schema_version != _MATRIX_SCHEMA_VERSION:
    ...
    window_indexes = {item.window["index"] for item in evaluations}
    if _required_int(payload, "candidate_count") != len(candidate_ids):
        raise ValueError("matrix candidate_count does not match evaluations")
    if _required_int(payload, "window_count") != len(window_indexes):
        raise ValueError("matrix window_count does not match evaluations")
    for evaluation in evaluations:
        _verify_evaluation_identity(evaluation, dataset_hash)
...
async def run_planned_evaluation(
    *,
    ...
    if (
        dataset.slice_semantic_sha256(window["train_start"], window["train_end"])
        != evaluation.train_semantic_sha256
    ):
        raise ValueError("planned train slice hash does not match the dataset")
    if (
        dataset.slice_semantic_sha256(window["test_start"], window["test_end"])
    ...
    with tempfile.TemporaryDirectory(prefix="llm-arbitrage-evaluation-") as temporary:
        ...
        test_path.write_bytes(
            b"".join(lines[window["test_start"] : window["test_end"]])
        )
        candidate_path.write_bytes(config_canonical_bytes(candidate))
    ...
    if (
        experiment.manifest.config_canonical_sha256
        != evaluation.candidate_config_sha256
    ):
    ...
    record = {
        ...
        "evidence_boundary": (
            "Only the planned test slice was replayed; no winner was selected."
        ),
    }
...
def load_evaluation_record(bundle_path: Path) -> dict[str, Any]:
    ...
    payload = _load_json_object(path, "evaluation record")
    schema_version = payload.get("schema_version")
...
def _parse_evaluation(value: Any) -> MatrixEvaluation:
    ...
    allowed = {
        ...
        "overrides",
        "window",
        "train_semantic_sha256",
        "test_semantic_sha256",
    }
    unknown = sorted(set(payload) - allowed)
    if unknown:
        raise ValueError(f"matrix evaluation contains unknown fields: {', '.join(unknown)}")
    ...
    if set(window_payload) != set(keys):
        raise ValueError("matrix evaluation window contains unexpected fields")
    ...
    if window["test_start"] - window["train_end"] != window["purge_size"]:
        raise ValueError("matrix evaluation purge_size is inconsistent")
    return MatrixEvaluation(
        ...
        window=window,
        train_semantic_sha256=_sha256(
            payload.get("train_semantic_sha256"),
            "train hash",
        ),
        test_semantic_sha256=_sha256(
            payload.get("test_semantic_sha256"),
            "test hash",
        ),
    )
...
def _verify_evaluation_identity(
    ...
    expected_candidate_id = f"candidate-{evaluation.candidate_config_sha256[:24]}"
    if evaluation.candidate_id != expected_candidate_id:
        raise ValueError("matrix candidate_id does not match the candidate hash")
    identity = {
        "schema_version": 1,
        "candidate_config_sha256": evaluation.candidate_config_sha256,
    ...
    if evaluation.evaluation_id != expected_evaluation_id:
        raise ValueError("matrix evaluation_id does not match its identity fields")
...
def _load_json_object(path: Path, description: str) -> dict[str, Any]:
    try:
        parsed = json.loads(
            ...
            parse_constant=lambda value: _raise_non_finite(description, value),
        )
    except _DuplicateJsonKey as error:
        raise ValueError(f"duplicate {description} JSON key: {error.key}") from error
...
def _raise_non_finite(description: str, value: str) -> None:
    raise ValueError(f"{description} contains a non-finite number: {value}")
