...
class ExperimentRegistry:
    ...
    def _create_schema(self) -> None:
        self._connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS lineage_nodes (
                ...
                dataset_semantic_sha256 TEXT NOT NULL,
                kind TEXT NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS experiments (
                ...
                lineage_id TEXT,
                dataset_semantic_sha256 TEXT NOT NULL,
                config_canonical_sha256 TEXT NOT NULL,
                manifest_json TEXT NOT NULL,
                replay_report_json TEXT NOT NULL,
                performance_report_json TEXT NOT NULL,
                imported_at TEXT NOT NULL,
                FOREIGN KEY (lineage_id) REFERENCES lineage_nodes(lineage_id)
            );
            ...
            CREATE TABLE IF NOT EXISTS evaluations (
                ...
                candidate_config_sha256 TEXT NOT NULL,
                test_semantic_sha256 TEXT NOT NULL,
                train_semantic_sha256 TEXT NOT NULL,
    ...
    def trust_public_key(
        ...
        *,
        ...
        identity = public_key_identity(load_public_key(path))
        row = self._connection.execute(
            "SELECT public_key_base64 FROM trusted_keys WHERE key_id = ?",
            (identity.key_id,),
        ).fetchone()
        if row is not None:
            if str(row["public_key_base64"]) != identity.public_key_base64:
        ...
        self._connection.execute(
            ...
            INSERT INTO trusted_keys (
                key_id, algorithm, public_key_base64, label, added_at
            ) VALUES (?, ?, ?, ?, ?)
            """,
            (
                identity.key_id,
    ...
    def import_lineage(self, path: Path) -> dict[str, Any]:
        ...
        for parent in manifest.parents:
            if not self._row_exists(
                "SELECT 1 FROM lineage_nodes WHERE lineage_id = ?",
                (parent,),
            ):
        ...
        with self._connection:
            self._connection.execute(
                ...
                INSERT INTO lineage_nodes (
                    lineage_id, dataset_semantic_sha256, kind,
            ...
            for parent in manifest.parents:
                self._connection.execute(
                    """
                    INSERT INTO lineage_edges (lineage_id, parent_lineage_id)
                    VALUES (?, ?)
                    """,
                    (manifest.lineage_id, parent),
                )
        return {
            "lineage_id": manifest.lineage_id,
            "status": "registered",
    ...
    def import_bundle(
        ...
        *,
        ...
        trusted_row = self._connection.execute(
            "SELECT public_key_base64 FROM trusted_keys WHERE key_id = ?",
            (verified.key_id,),
        ).fetchone()
        trusted = trusted_row is not None
        if trusted_row is not None and (
            str(trusted_row["public_key_base64"]) != verified.public_key_base64
        ):
            raise RuntimeError("trusted key ID collision")
        if not trusted and not allow_untrusted:
            raise PermissionError(
                f"attestation signer is not trusted: {verified.key_id}"
            )
        ...
        if lineage_id is not None:
            lineage_row = self._connection.execute(
                """
                SELECT dataset_semantic_sha256 FROM lineage_nodes
                WHERE lineage_id = ?
                """,
                (lineage_id,),
            ).fetchone()
            ...
            if str(lineage_row["dataset_semantic_sha256"]) != dataset_hash:
                raise ValueError(
                    "attested lineage dataset hash does not match bundle"
                )

        existing = self._connection.execute(
            """
            SELECT bundle_root_sha256, signer_key_id FROM experiments
            WHERE experiment_id = ?
            """,
            (experiment_id,),
        ).fetchone()
        if existing is not None:
            if (
                str(existing["bundle_root_sha256"])
                != verified.bundle_root_sha256
                or str(existing["signer_key_id"]) != verified.key_id
            ):
                raise RuntimeError(
                    "experiment ID conflicts with immutable evidence"
                )
            return {
        ...
        replay = json_object(
            (bundle / "replay_report.json").read_text(encoding="utf-8")
        )
        performance = json_object(
            (bundle / "performance_report.json").read_text(encoding="utf-8")
        )
        self._connection.execute(
            """
            INSERT INTO experiments (
                experiment_id, run_id, bundle_path, bundle_root_sha256,
                signer_key_id, trusted, lineage_id,
                dataset_semantic_sha256, config_canonical_sha256,
                manifest_json, replay_report_json, performance_report_json,
                imported_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
    ...
    def register_evaluation(
        ...
        *,
        ...
        bundle = bundle_path.resolve()
        record = load_evaluation_record(bundle)
        manifest = json_object(
            (bundle / "manifest.json").read_text(encoding="utf-8")
        )
        manifest_experiment_id = required_string(manifest, "experiment_id")
        manifest_run_id = required_string(manifest, "run_id")
        manifest_dataset_hash = nested_string(
            manifest,
            "dataset",
            "semantic_sha256",
        )
        manifest_config_hash = nested_string(
            manifest,
            "configuration",
            "canonical_sha256",
        )
        expected_values: dict[str, Any] = {
            ...
            "source_dataset_semantic_sha256": matrix.dataset_semantic_sha256,
        ...
        for key, expected in expected_values.items():
            if record.get(key) != expected:
                raise ValueError(
                    f"bundle evaluation {key} does not match matrix plan"
                )
        if manifest_dataset_hash != planned.test_semantic_sha256:
            raise ValueError("experiment dataset is not the planned test slice")
        if manifest_config_hash != planned.candidate_config_sha256:
        ...
        if experiment_id != manifest_experiment_id:
            raise RuntimeError("registered experiment ID changed during import")

        existing = self._connection.execute(
            "SELECT experiment_id FROM evaluations WHERE evaluation_id = ?",
            (evaluation_id,),
        ).fetchone()
        if existing is not None:
        ...
        self._connection.execute(
            ...
            INSERT INTO evaluations (
                evaluation_id, experiment_id, matrix_sha256, candidate_id,
                candidate_config_sha256, test_semantic_sha256,
                train_semantic_sha256, window_index, evaluation_json,
                registered_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
    ...
    def evaluation_rows(
        ...
        rows = self._connection.execute(
            """
            SELECT e.*, x.performance_report_json,
                   x.replay_report_json, x.trusted
        ...
        return tuple(
            {
                ...
                "performance_report": json_object(
                    str(row["performance_report_json"])
                ),
    ...
    def verify(self) -> RegistrySummary:
        ...
        if integrity_row is None:
            raise RuntimeError("registry did not return an integrity result")
        ...
        if self._connection.execute("PRAGMA foreign_key_check").fetchall():
            raise RuntimeError("registry foreign-key check failed")
        untrusted_reference = self._connection.execute(
            """
            SELECT 1 FROM experiments AS e
            LEFT JOIN trusted_keys AS k ON e.signer_key_id = k.key_id
            WHERE e.trusted = 1 AND k.key_id IS NULL
            LIMIT 1
            """
        ).fetchone()
        if untrusted_reference is not None:
            raise RuntimeError("trusted experiment references a missing key")
        return RegistrySummary(
            integrity=integrity,
            trusted_keys=self._count("trusted_keys"),
            lineage_nodes=self._count("lineage_nodes"),
            lineage_edges=self._count("lineage_edges"),
            experiments=self._count("experiments"),
            evaluations=self._count("evaluations"),
        )
    ...
    def _count(self, table: str) -> int:
        ...
        if row is None:
            raise RuntimeError(f"registry count failed for table: {table}")
        return int(row[0])
