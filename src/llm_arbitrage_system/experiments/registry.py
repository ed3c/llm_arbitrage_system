    ...
    def register_evaluation(
        ...
        *,
        ...
        if record.get("experiment_id") != experiment_id:
            raise ValueError(
                "bundle evaluation experiment_id does not match the bundle"
            )
        experiment_row = self._connection.execute(
            """
            SELECT run_id, dataset_semantic_sha256, config_canonical_sha256
        ...
        if record.get("run_id") != str(experiment_row["run_id"]):
            raise ValueError("bundle evaluation run_id does not match the bundle")
        if (
            str(experiment_row["dataset_semantic_sha256"])
            != planned.test_semantic_sha256
        ):
            raise ValueError("experiment dataset is not the planned test slice")
        if (
            str(experiment_row["config_canonical_sha256"])
