...
def aggregate_registry_matrix(
    ...
    planned: dict[str, list[MatrixEvaluation]] = defaultdict(list)
    ...
    for candidate_id in sorted(planned):
        expected = sorted(
            ...
            key=lambda item: (item.window["index"], item.evaluation_id),
        )
        rows = [
            rows_by_id[item.evaluation_id]
            for item in expected
            if item.evaluation_id in rows_by_id
        ]
        missing = [
            item.evaluation_id
            for item in expected
            if item.evaluation_id not in rows_by_id
        ]
        ...
        for row in rows:
            ...
            failed += _integer(metrics.get("failed_plan_count"))
            failed += _integer(metrics.get("partially_filled_plan_count"))
        hashes = {item.candidate_config_sha256 for item in expected}
        if len(hashes) != 1:
            raise ValueError(
                f"candidate has inconsistent config hashes: {candidate_id}"
            )
        candidates.append(
            {
                ...
                "trusted_evaluations": sum(
                    bool(row["trusted"]) for row in rows
                ),
    ...
    return {
        ...
        "complete_candidate_count": sum(
            item["coverage"] == "complete" for item in candidates
        ),
        "partial_candidate_count": sum(
            item["coverage"] == "partial" for item in candidates
        ),
        "missing_candidate_count": sum(
            item["coverage"] == "none" for item in candidates
        ),
        ...
        "alpha_decay": None,
        "evidence_boundary": (
            "This report aggregates registered test-window execution evidence. "
            "It does not select a winner, estimate realized PnL, or establish "
            "alpha decay."
        ),
    }
...
def _append_float(values: list[float], value: Any) -> None:
    ...
    if not isfinite(parsed):
        raise ValueError("performance rate must be finite")
    values.append(parsed)
