...
`run-evaluation` executes exactly one matrix test slice. `registry-register-evaluation` verifies its candidate, config, window, and test-slice binding. `registry-aggregate` reports complete, partial, and missing coverage without choosing a winner.
