...
Registration recomputes the matrix identity and rejects a bundle unless the evaluation ID, candidate configuration hash, test dataset hash, train hash, indexes, experiment manifest, and trusted signature all match. The train slice is evidence for the plan only; it is not replayed by the Phase 4 test runner.
